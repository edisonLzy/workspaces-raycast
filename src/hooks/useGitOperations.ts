    ...
    async (params: CreateWorktreeParams) => {
      ...
      try {
        ...
        if (pathExists) {
          ...
          // 删除已存在的路径
          await execUtils.execGit(["worktree", "remove", "--force", targetPath], {
            cwd: params.repoPath,
            timeout: 10000,
          });
        }
        ...
        // 3. 检查分支是否已存在
        const branchExists = await checkBranchExists(params.repoPath, params.branch);
        if (branchExists) {
          throw new Error(`分支 ${params.branch} 已存在,请使用其他名称`);
        }

        // 4. 执行 git worktree add (调用纯工具函数)
        await execUtils.execGit(["worktree", "add", "-b", params.branch, targetPath], {
          cwd: params.repoPath,
          timeout: 30000,
        });
